...
/* 	Plugin Name: Contact Us By Lord Linus
	...
	Version: 2.6
	Author: lordlinus
	Author URI: http://impulsesoftech.com/contact-us
	Licence: GPVl
*/
?>
<?php
...
class contact_class
{
	...
	function contact_show_form($params = '')
	{
		...
		if ( empty($result) || (!empty($result) && !$this->o['hideform']) )
		{
			if ( !empty($_POST['cuf_subject'.$n]) )
				$cuf_subject = htmlspecialchars($_POST['cuf_subject'.$n]);
				...
				$cuf_subject = htmlspecialchars($_GET['subject']);
	...
	function sendMail( $n = '', $params = '' )
	{
		...
		$o = get_option('lorlinus_contact_us_form');
		$result = '';
		$to = htmlspecialchars($o['to_email']);
		
		$from	= htmlspecialchars($o['from_email']);
	
		$name	= htmlspecialchars($_POST['cuf_sender'.$n]);
		$email	= htmlspecialchars($_POST['cuf_email'.$n]);
		$subject= htmlspecialchars($_POST['cuf_subject'.$n]);
		$msg	= htmlspecialchars($_POST['cuf_msg'.$n]);
